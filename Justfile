...
demo-recipe recipe features='' target='' branch='':
    @[ -n "${NETBOX_TOKEN:-}" ] || { set -a; [ ! -f .env ] || . ./.env; set +a; }; python3 -m estates demo --recipe {{quote(recipe)}} --features {{quote(features)}} {{if target == '' { '' } else { '--target ' + quote(target) + ' --branch ' + quote(branch) } }}
