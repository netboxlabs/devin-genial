...
| 2026-09-20 | MSP, tenancy/operator proofs + drift, growth WITH the second sheet, closeout (Pinnacle) | 4.7.1 local, shared with 20 branches | Prep & closeout yes (151 s to live, every claim verified); growth **no as documented** — S1 | run #28's growth block was destructive-then-broken: default --out collided with v1's directory AFTER the printed retire ran (fixed: growth defaults to `<namespace>-v2`, the block is offline-first — nothing retires until the grown compose passes every gate — and `out` is a Justfile parameter); the grown sheet lost the profile's headline claim (fixed: hooks survive --recipe; the bank's merger clause is a graph fact); grown sheets state what changed (New-in-this-growth row); template table was uniformly +6 stale; MSP page-cap proof annotated |
| 2026-09-20 | Manufacturing, Juniper + OT + drift, growth-with-second-sheet replayed literally after the S1 fix (Ironvale) | 4.7.1 local, shared with 20 branches | Prep yes (call-ready T+4:22); growth yes-with-papercuts; closeout yes (~11 min lifecycle; offline-first order proved safe — "nothing was ever at risk"; growth exactness API-verified: one new site, zero removals, all 359 IPs unchanged) | the offline sheet's recompose one-liner re-derived template flags and would rebuild the stock estate under a "same estate" claim (fixed: it carries --recipe/--previous); grown sheets suggest a distinct v2 branch name; the drift-anchor-vs-new-site split now explains itself |
| 2026-09-20 | Enterprise DC, customer app names via demo-recipe, rack-independence + power what-if, growth (Halberd) | 4.7.1 local, shared with 20 branches | Prep yes-with-papercuts; what-if yes; growth **no as documented** — S1 (call-ready T+4:01; loads/readbacks flawless; rack independence 18/18 groups proven live) | the grown sheet's close-out still used the v1 branch name — run #30's derived branch was applied to the go-live block but not the retire/verify lines (fixed: one derivation feeds every command line, pinned by a per-line test); the enterprise failure-domain step hardcoded the stock `payments` key (fixed: graph-fact workload name); the -v2 default dead-ended the NEXT growth (fixed: first free -vN generation directory) |
